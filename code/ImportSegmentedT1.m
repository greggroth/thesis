...
%
%   Output: head data as single with variables stored in the 4th
%   dimension.
%
...
for i = 1:5
    ...
    %  move structure data to new matrix   
    holder(:,:,:,1) = mask;        
    %  get indicies of tissues              
    a = find(holder(:,:,:,1) == 1);          
    %  gets coordinates from index    
    [x y z t] = ind2sub(size(holder),a);         
