...
\end{equation}
\hl{After rearranging to solve for $\frac{dT}{dt}$ an substituting values in, it is found that the change in temperature is approximately 0.07 K/s:}
\begin{align}
...
\end{align}
\hl{where A and V are the surface area and volume of the voxel, $\sigma$ is the Stefan–Boltzmann constant and $\rho_{gm}$ is the density of gray matter.  The $1/6$ factor is there because only one face of the voxel is exposed to air.  Radiation passing through the other faces will be reabsorbed by the tissue}~(\cref{fig:waterabs}).
